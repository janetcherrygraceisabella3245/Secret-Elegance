...
export default function BookCompanion() {
    ...
    const handleSelect = (id: number, name: string, rate: number) => {
        updateBookingState({
            caregiverId: id,
            caregiverName: name,
            hourlyRate: rate
    ...
    return (
        <div className="min-h-screen w-full flex flex-col bg-[#FFF5F7]">
            ...
            <div className="fixed top-16 inset-x-0 h-0.5 bg-gray-100 z-10">
                <div className="h-full bg-[#E8798A] transition-all duration-500" style={{ width: `${progress}%` }} />
            </div>

            <main className="flex-1 mt-16.5 px-6 md:px-16 lg:px-24 py-12">
                        ...
                        return (
                            <div key={girl.id} className="group bg-white rounded-3xl overflow-hidden shadow-sm border-2 border-transparent hover:border-rose-100 hover:shadow-xl transition-all duration-300">
                                <div className="relative aspect-3/4 overflow-hidden bg-rose-50 cursor-pointer" onClick={() => handleViewProfile(girl.id)}>
                                    <img src={girl.image} alt={girl.fullName} className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-110" />
                                    {isSelected && (
                                ...
                                <div className="p-6">
                                    <div className="flex justify-between items-start mb-2">
                                        ...
                                        <span className="text-sm text-gray-400">{girl.age} </span>
                                    </div>
                                    ...
                                    <div className="flex gap-3">
                                        ...
                                        <button
                                            onClick={() => handleSelect(girl.id, girl.fullName, girl.hourlyRate)}
