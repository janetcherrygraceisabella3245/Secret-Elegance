...
export default function BookDate() {
  ...
  const serviceLabel = bookingState.service || "Caregiving Service"
  const caregiverLabel = bookingState.caregiverName || "Your Caregiver"
  ...
  return (
    <div className="min-h-screen w-full flex flex-col bg-white">
      ...
      <header className="fixed top-0 inset-x-0 h-16 bg-white border-b border-gray-100 flex items-center justify-between px-8 z-10">
        ...
        <div className="text-sm font-medium tracking-wide text-[#2D3142] uppercase">
          Choose Date
        </div>
      ...
      <div className="fixed top-16 inset-x-0 h-[2px] bg-gray-100 z-10">
        <div
          className="h-full bg-[#E8798A] transition-all duration-500"
      ...
      <main className="flex-1 mt-[66px] flex flex-col md:flex-row">
        ...
        <div className="w-full md:w-1/2 bg-[#2D3142] text-white p-12 md:p-20 flex flex-col relative min-h-[calc(100vh-66px)]">
          <div className="flex-1 max-w-md mx-auto w-full flex flex-col mt-10">
            <h3 className="font-serif italic text-[#E8798A] text-xl mb-12">
            ...
            <div className="space-y-8">
              ...
              {bookingState.caregiverName && (
                <div className="flex items-start">
                  ...
                  <div>
                    <div className="text-sm text-gray-400 mb-1">Caregiver</div>
                    <div className="font-serif text-2xl">{caregiverLabel}</div>
                  </div>
                </div>
              )}
            </div>
            ...
            <blockquote className="font-serif italic text-2xl leading-relaxed text-gray-300">
              "We bring dignity and warmth to every moment of care."
            </blockquote>
          </div>
        ...
        <div className="w-full md:w-1/2 bg-[#FFF5F7] p-12 md:p-20 flex flex-col min-h-[calc(100vh-66px)]">
          <div className="max-w-md mx-auto w-full flex flex-col h-full mt-10">
            ...
            <p className="text-gray-500 mb-12">
              Select an available date to begin service.
            </p>
            ...
            <div className="bg-white rounded-3xl p-8 shadow-sm border border-rose-50 mb-auto">
              ...
              <div className="flex items-center justify-between mb-8">
                ...
                <div className="font-serif text-2xl text-[#2D3142] font-medium">
                  {monthLabel}
                </div>
                <button
                  onClick={nextMonth}
              ...
              <div className="grid grid-cols-7 mb-4">
                {["Su", "Mo", "Tu", "We", "Th", "Fr", "Sa"].map((d) => (
                  <div
                    key={d}
                    className="text-center text-xs font-medium text-gray-400 uppercase tracking-wider"
                  >
                    {d}
                  </div>
                ))}
              ...
              <div className="grid grid-cols-7 gap-y-4 gap-x-2">
                {grid.map(({ date, isCurrentMonth }, idx) => {
                  ...
                  const isSelected =
                    isCurrentMonth &&
                    selectedDate &&
                    startOfDay(selectedDate).getTime() === dayStart.getTime()
                      ...
                      <button
                        ...
                        className={[
                          ...
                          !isCurrentMonth
                            ...
                            : isSelected
                            ? "bg-[#E8798A] text-white shadow-md shadow-rose-200"
                            : isTodayDate
                            ? "text-[#E8798A] hover:bg-rose-50"
                            : "text-[#2D3142] hover:bg-gray-100",
                        ].join(" ")}
                      >
                        {isTodayDate && !isSelected && (
                          <span className="absolute inset-0 rounded-full border border-[#E8798A]" />
                        )}
                        {date.getDate()}
                      </button>
                    </div>
            ...
            <div className="mt-12 flex flex-col space-y-6">
              <div className="flex items-center space-x-3 text-sm">
                <CalendarIcon
                  className={`w-5 h-5 ${selectedDate ? "text-[#E8798A]" : "text-gray-300"}`}
                />
                {selectedDate ? (
